...
if __name__ == '__main__':
    try:
        ...
        new_dataset_id = int(input('Ilastik output Dataset ID: '))
        new_dataset = omero.get_dataset(conn, new_dataset_id)

        images = dataset.listChildren()
        images_names_ids = {i.getName(): i.getId() for i in images}
        image_root_names = list(set([n[:-4] for n in images_names_ids.keys()]))

        prob_images = new_dataset.listChildren()
        prob_images_names_ids = {i.getName(): i.getId() for i in prob_images}
        ...
        for counter, image_root_name in enumerate(image_root_names):
            ...
            prob_image = conn.getObject('Image', prob_images_names_ids[f'{image_root_name}_MIP_PROB'])
            prob_data = omero.get_intensities(prob_image)
