    ...
    profiles = {
        "z": plot_profile(
            xz, profile_z, fit_z, "Z profile", "r", "g", {
                "FWHM": f"{fwhm_z * pixel_size[0]:.2f}",
                "R^2": f"{cd_r2_z:.2f}",
            }
        ),
        "y": plot_profile(
            xy, profile_y, fit_y, "Y profile", "r", "g", {
                "FWHM": f"{fwhm_y * pixel_size[1]:.2f}",
                "R^2": f"{cd_r2_y:.3f}",
            }
        ),
        "x": plot_profile(
            xx, profile_x, fit_x, "X profile", "r", "g", {
                "FWHM": f"{fwhm_x * pixel_size[2]:.2f}",
                "R^2": f"{cd_r2_x:.3f}",
            }
        )
    }
    ...
    for image in images:
        ...
        for roi in rois:
            ...
            if isinstance(shape, Point):
                ...
                zx_slice = rescale(zx_slice, (pixel_size[0] / pixel_size[2], 1), anti_aliasing=True, preserve_range=True)
                ...
                zy_slice = rescale(zy_slice, (pixel_size[0] / pixel_size[1], 1), anti_aliasing=True, preserve_range=True)
                ...
                for axis, profile_img in profiles.items():
                    ...
                    profile_image = ot.create_image_from_numpy_array(
                        ...
                        image_name=f"{image.getName()}_{shape_comment}_{axis}_profile",
